...
| Token project on Solana | Real | Added 2026-09-24. Picking Token offers a chain (Ethereum/Polygon/BSC or Solana) and the matching networks; a Solana token project opens the Launchpad's Solana tab (`?chain=solana`), restores/autosaves its draft there, and links to the SPL token once launched — shown on its dashboard card. A project's network must now belong to its chain (checked on create and every update). |
...
| Advanced ERC-20 owner tools ("Manage" on an `erc20_advanced` history row) | Real | Added 2026-09-24. Live settings read from the contract; the owner wallet sends enable trading, tax rates (entered in %, sent in basis points, max 10%), max transaction/wallet (whole tokens), fee wallets, fee exclusion, trading-pair registration, and renounce ownership (only once trading is enabled, behind a can't-be-undone confirmation). Contracts deployed before the template fix are detected (no `marketPairs`) and told plainly that their taxes don't apply to DEX trades. |
| DEX liquidity ("Liquidity" on an ERC-20 history row) | Real | Added 2026-09-25. Uniswap V2 on Ethereum/Sepolia/Polygon, PancakeSwap V2 on BSC/BSC Testnet — every router/factory/wrapped-native address checked against its live chain; none on Polygon Amoy (no V2 deployment we could confirm). The first add creates the pool at exactly the amounts given (a pool someone seeded meanwhile at another price makes it revert); later adds go in at the pool's price with 1% slippage. `POST /api/contracts/deployments/<id>/liquidity` records an add only if the receipt succeeded, went to that network's router from one of the account's wallets, and the token's own pair emitted a Mint — amounts are taken from that event. For erc20_advanced, the panel offers "Register as trading pair" and says when trading isn't enabled yet. Removing: any whole-percent share of the wallet's LP position, at the pool's ratio with 1% slippage (approve LP tokens, then `removeLiquidityETHSupportingFeeOnTransferTokens`); for an advanced token the panel shows the buy tax the pool→router hop pays once the pool is a registered pair, and refuses a removal larger than the token's transfer limits before anything is sent. Removals aren't recorded by the backend. Not offered: locking LP tokens, Solana pools. |
| LP time-locks (Liquidity panel → "Lock LP tokens"; Token Time-Lock template) | Real | Added 2026-09-25. EVM has no first-party locker like Raydium's, so each lock is its own tiny contract deployed by the locker's wallet from the `token_timelock` template: token, beneficiary and release time are compile-time constants; no owner, no admin, no early exit; `release()` (anyone may call) pays only the beneficiary, only once the chain's time has passed the release time. The Liquidity panel deploys one for the pool's LP token with the connected wallet as beneficiary, moves a share of the wallet's LP into it, lists the pool's locks with what each holds and until when, and shows the time-locked share; the lock's own history row shows its state and releases it. Source-verifiable like any template. The app never holds LP. Unlock state is judged by the latest block's timestamp, not the device clock. |
| Solana liquidity ("Liquidity" on a Solana token history row) | Real (needs the sidecar) | Added 2026-09-25. Raydium CPMM, token against SOL, standard 0.25% fee tier — program, fee config and pool-fee receiver from the Raydium SDK's constants, each checked on devnet and mainnet. Create (exact seed amounts; fails if the pool already exists), deposit (up to the token amount given — LP-unit rounding can leave a few base units — plus the pool's SOL ratio, at most 1% more), withdraw (whole-percent share of the wallet's LP). The pool address is derived from the mint, never taken from a client. `POST /api/solana-tokens/<id>/pool/record` stores an action only if the transaction succeeded, was paid by one of the account's Solana wallets, ran the CPMM program, and moved both of this pool's vaults the same way (swaps aren't recorded); amounts are the vault changes. Warns when the token still has a freeze authority (Raydium allows it; buyers treat it as a red flag). Lock (added 2026-09-25): Raydium's Burn & Earn — permanent, no unlock exists — behind a can't-be-undone confirmation; the locker gets a Fee Key NFT for the position's trading fees, and every viewer sees the share of the pool locked (the lock authority's LP balance). Recorded as a lock only if the lock program ran and the lock authority newly holds this pool's LP. Not offered: claiming Fee Key fees in-app (done on Raydium), other fee tiers, pairs other than SOL. |
| Solana SPL token launch (Token Launchpad → "Solana") | Real (optional config) | Added 2026-09-24. Classic SPL token + Metaplex Token Metadata (name, symbol, optional description/logo), built by `services/candy-machine/src/routes/token.ts` in one transaction; your connected Solana wallet signs, pays, receives the full supply, and keeps update authority. Mint and freeze authority are revoked by default (fixed supply, no freezing holders), each opt-out-able. Recording re-verifies on-chain (success, fee payer = your wallet, mint referenced) and reads decimals/supply/authorities back from the mint account itself. Optional config: a description or logo needs Pinata (clean 503 otherwise); a name+symbol-only token needs no IPFS at all. Linked to Projects since 2026-09-24: a Token project can be created on Solana from the wizard; it opens on this tab, restores and autosaves its draft, and links to the launch once recorded. |
...
| Solana token owner tools ("Manage" on a history row) | Real | Added 2026-09-24. For a token launched with an authority kept: mint more (whole tokens, into the authority's wallet), fix the supply (revoke mint authority) or revoke the freeze authority — revokes behind an explicit can't-be-undone confirmation. Built for and signed by the mint's *current* on-chain authority (which may not be the launch-time creator); supply and badges are re-read from the chain after every action. The SPL Token program itself rejects anyone else (verified: owner mismatch) and any mint after the supply is fixed. |
| Edit token details / lock metadata (Manage → Details) | Real | Added 2026-09-24. Change a launched token's name, symbol, description and logo — or lock its metadata permanently (immutable, behind a can't-be-undone confirmation). Starts from what's on-chain plus the off-chain JSON's description/logo; a description/logo change re-pins that JSON (keeping the current logo unless replaced). Signed by the token's update authority — the Token Metadata program rejects anyone else, and any change once locked (both verified). The off-chain JSON is only fetched server-side from this app's own IPFS gateway (a creator-controlled URI anywhere else would let anyone point the backend at internal addresses). |
...
| Trait rules ("Trait rules" panel, between layers and generation) | Real | Added 2026-09-24. "A never appears with B" and "A requires B" between traits on different layers. Generation samples only rule-abiding combinations (each layer's candidates filtered by what's already picked — not generate-and-reject), and the "unique combinations possible" count is the rule-aware one computed by the server, so asking for more than the rules allow fails up front. Same-layer, duplicate, and contradictory rules are refused; deleting a trait or layer removes its rules. |
...
| Manage a deployed ERC-721 ("Manage" on `/nft/deploy-evm`) | Real | Added 2026-09-24. Live on-chain state (minted/max, price, per-wallet limit, minting on/off, base URI, proceeds held by the contract) and the template's owner-only functions: withdraw proceeds, pause/resume minting, change price (entered in the native token) and per-wallet limit, and re-pin metadata + update the base URI. Read and sent directly by the owner's wallet — no app-side copy to go stale; non-owners see it read-only. Max supply is fixed by the template. Withdraw uses the template's fixed-gas `transfer`, so the owner must be a regular wallet, not a smart-contract wallet (stated in the panel). |
...
| Action | Status | Notes |
|---|---|---|
| Launch a Candy Machine from a published NFT collection | Real | Real `@metaplex-foundation/umi` + `mpl-core-candy-machine`/`mpl-core` on-chain creation (Core Collection + guarded Core Candy Machine + config lines) via `services/candy-machine` — migrated 2026-08-19 from the legacy `mpl-candy-machine` stack after its Guard↔Core CPI was found broken everywhere (Metaplex deprecated it); verified end to end against real devnet transactions, not just typechecked. Your connected Solana wallet signs and pays for everything — the sidecar generates only single-use, in-memory ephemeral signers for the two brand-new accounts, never a platform authority key. Up to 10,000 items per drop (the generator's own cap; was 20 until 2026-09-24): the collection's metadata is pinned as one IPFS folder and each item stored on-chain as just its number + `<n>.json` under a shared name prefix and folder URI; the creation transaction carries the first items and the rest load in batches your wallet approves with one prompt each (`signAllTransactions`), resumable from the chain's own count if interrupted ("Resume launch"). A drop is only recorded once every item is loaded on-chain. Item names (`<collection name> #<n>`) must fit Solana's 32 bytes — a longer collection name is refused before anything is paid. |
...
| Max mints per wallet (optional; at launch or via "Edit phases") | Real | Added 2026-09-24. The Candy Guard `mintLimit` guard, in the default guard set so one on-chain counter covers every phase — a wallet's allowlist and public mints count against the same limit. Checked against the chain at record/edit time like the rest of the guard config. The storefront shows "Limit N per wallet" and, for a connected wallet, its on-chain count; at the limit it shows why instead of a mint button, and the backend refuses to build the mint (403). |
