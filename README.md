...
- The automated accessibility scan (color contrast, ARIA, labels) is complemented by a real keyboard-interaction + accessibility-tree pass (2026-09-15) — driving real `Tab`/`Escape` sequences and reading Chromium's real AX tree (`Accessibility.getFullAXTree`, the same data a screen reader consumes) found and fixed 4 real bugs: an overly-verbose link name on the homepage's feature cards, and three places (the mobile nav drawer, the shared dropdown menu, the delete-confirmation dialog) where closing an overlay via Escape dropped keyboard focus to `<body>` instead of returning it to the trigger. What's genuinely still open: actual screen-reader *rendering* (how NVDA/JAWS/VoiceOver phrase and pace an announcement) needs a real person with real assistive tech, not a browser automation substitute.
