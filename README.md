...
**Verification status, read before trusting any of this in production**: both the EVM token/contract deploy flow *and* the Solana Candy Machine flow (launch + public mint) now have real end-to-end browser coverage (`frontend/e2e/`) — a real backend, a real Solidity compile / real Metaplex program calls, a real local chain (`anvil` / `solana-test-validator`), a real wallet signature, driven through an actual Chromium browser in CI on every push. These passes caught and fixed several real bugs that had survived every prior layer of testing: compiled bytecode missing its required `"0x"` prefix (every real EVM deployment was reverting on-chain), a stale-closure bug that left a successful deployment stuck showing "recording…" forever in the UI, and a Solana commitment-level mismatch (`"confirmed"` vs. solana-py's default `"finalized"`) that could make the backend fail to find a transaction the frontend had already confirmed. See `docs/REBUILD_PROGRESS.md`'s two "Testing & CI" entries for the full writeup. The Candy Machine launch flow's blockhash-expiry issue (`docs/CANDY_MACHINE_BLOCKHASH_FIX_SPEC.md`) is code, but not yet devnet-click-through-verified — a fast local validator can't reproduce the real network latency between wallet approvals the fix is actually for, so that's still the next thing to confirm before trusting it in production.
