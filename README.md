...
- `web3`/`eth-account` and `solana`/`solders` are deliberately held back at major-version-old pins — not "not yet gotten to," but confirmed genuinely blocked: `web3`'s newer async websocket provider needs `websockets>=13`, which conflicts outright (unresolvable by a clean install, not just untested) with `solana==0.34.3`'s `websockets<12` pin; and newer `solana`/`solders` remove the synchronous RPC client this app's whole Solana integration is built on, which needs a real rewrite (async Flask routes or wrapping every call site), not a routine bump. Both need to move together as one dedicated pass — see `docs/REBUILD_PROGRESS.md`'s 2026-09-11 entries for the exact versions tried and why each was reverted. `flask-limiter`, `flask-cors`, and `openai` were bumped in the same pass and are current.
