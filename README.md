...
- **Contract source verification** — one click submits a deployed contract's exact compiler input to Etherscan (Polygonscan/BscScan and testnets via Etherscan's multichain API, one optional key), polls until it settles, and links to the verified code.
- **Solana token management** — after launch: mint more, fix the supply, revoke the freeze authority, rename or re-describe the token (new logo too), or lock its metadata for good — each signed by the relevant on-chain authority and re-read from the chain afterwards.
- **Solana token launch** — the Token Launchpad's "Solana" tab creates a real SPL token with Metaplex Token Metadata (name, symbol, optional description and logo pinned to IPFS), signed and paid for by the creator's own wallet, which receives the full supply. Mint and freeze authority are revoked by default. Recorded only after the backend re-verifies the transaction and reads the mint account back from the chain. Devnet-first, same mainnet-confirmation gate as everywhere else.
- **Solana liquidity (Raydium)** — "Liquidity" on a launched SPL token: create its Raydium CPMM pool against SOL (0.25% fee tier; Raydium's 0.15 SOL pool-creation fee shown up front), add to it at the pool's price, withdraw a share of your position, or lock liquidity for good with Raydium's Burn & Earn (the pool shows everyone what share is locked) — each signed by your own wallet, and recorded only after the backend reads what the pool's vaults did in that transaction.
- **Public token pages** — a shareable link for any token launched here (`/token/<network>/<address>`, "Public page" on each history row): supply, whether the creator can still mint, freeze or change settings, taxes and trading status, the pool and its price, and how much liquidity is locked (and until when) — read from the chain when the page loads, no wallet or sign-in needed.
- **DEX liquidity** — "Liquidity" on any ERC-20 deployed here: create its pool on the network's Uniswap V2 (PancakeSwap V2 on BSC) paired with the native coin, or add to it at the current price, from the creator's own wallet (approve, then add). An advanced token's new pool can be registered as its trading pair in one click so buys and sells are taxed. Each add is recorded only after the backend reads it back from the chain. Remove any share of your position from the same panel, or time-lock LP tokens until a date in a small, owner-less lock contract deployed from your own wallet (the **Token Time-Lock** template — also deployable on its own from the Contracts Hub for any ERC-20).
- **Advanced ERC-20 owner tools** — "Manage" on an `erc20_advanced` deployment: enable trading (holders can't transfer until you do), change buy/sell taxes (capped at 10%), limits and fee wallets, exclude wallets from fees, register the DEX pair(s) buys and sells are taxed through, and renounce ownership — read live from the contract and sent by the owner's wallet.
...
- **NFT collections on EVM chains** — a published NFT Generator collection can also be deployed as an ERC-721 on Ethereum, Polygon or BSC: its metadata is pinned as one IPFS folder the contract's `tokenURI` resolves into, the creator's wallet deploys and owns the contract, public minting is switched on with a real owner transaction when they're ready, and an owner panel withdraws mint proceeds and changes price, limits, minting status and the metadata link.
- **NFT Collection Generator** — layer/trait upload (single or bulk, with rename/reweight/delete and layer reordering afterward — no more starting over on a mistake), rarity-weighted generation with real PIL image compositing (not placeholder images; runs as a background job with live progress, capped at 10,000 items/call), trait rules ("never appears with" / "requires") honored by generation, a real rarity-distribution view of what a generation run actually produced, publish-to-IPFS via Pinata, an inline AI-assisted rarity suggestion (optional GPT-4o vision call, real CV analysis runs regardless) during trait upload, and a standalone bulk AI trait analyzer for previewing a whole batch's rarity/diversity spread before uploading anything.
- **Solana Candy Machine — creator flow & public mint storefront** — launch a real Core Candy Machine drop of up to 10,000 items from a published NFT collection (items loaded in wallet-approved batches, resumable if interrupted) via the `services/candy-machine` sidecar, then share a public link any visitor can mint from with their own wallet, optionally with an allowlist phase (its own price and start time, enforced on-chain by Candy Guard groups) before public minting opens and an optional per-wallet mint limit — both editable after launch — and follow every drop's live sales and revenue on a creator dashboard. Testnet-first (Solana Devnet default), same mainnet-confirmation pattern as the EVM side.
...
- **Automated tests & CI** — pytest (backend, 280+ tests, including a real Redis service container backing the rate-limit storage tests) and Vitest (frontend, 160+ tests), plus real Playwright end-to-end tests for the EVM deploy flow (against a local `anvil` chain), the Solana Candy Machine (including allowlist phases) and SPL token launch flows (against a local `solana-test-validator`, real Metaplex programs cloned onto it), the NFT Generator's own multi-step upload UI and its ERC-721 deploy (read back on-chain, down to each token's `tokenURI` resolving to the right metadata) including real edit/delete/reorder round trips, and a real `@axe-core/playwright` WCAG 2 A/AA scan of every page — see `frontend/e2e/`. Four GitHub Actions jobs (backend, frontend, e2e, Candy Machine sidecar) on every push/PR to `main`.
...
- ~~The AI Trait Identifier (real third-party OpenAI calls) doesn't have end-to-end browser coverage~~ — closed 2026-09-22: covered via a local OpenAI stub (same pattern as the existing Pinata stub), both the inline per-trait suggestion during upload and the standalone bulk analyzer, see `frontend/e2e/README.md`. The model's actual classification *quality* isn't and can't be verified by browser automation — only that the integration itself is real.
- The automated accessibility scan (color contrast, ARIA, labels) is complemented by a real keyboard-interaction + accessibility-tree pass (2026-09-15) — driving real `Tab`/`Escape` sequences and reading Chromium's real AX tree (`Accessibility.getFullAXTree`, the same data a screen reader consumes) found and fixed 5 real bugs: an overly-verbose link name on the homepage's feature cards; three places (the mobile nav drawer, the shared dropdown menu, the delete-confirmation dialog) where closing an overlay via Escape dropped keyboard focus to `<body>` instead of returning it to the trigger; and every one of this app's ~15 inline error messages plus two real-time progress labels (NFT generation, Candy Machine launch) updating silently with no `role="alert"`/`aria-live` for a screen reader to notice. What's genuinely still open: actual screen-reader *rendering* (how NVDA/JAWS/VoiceOver phrase and pace an announcement) needs a real person with real assistive tech, not a browser automation substitute.
...
- NFT generation runs as a background job now (`backend/app/services/nft_generation_jobs.py`, a real Python thread coordinating through the job's DB row — not a broker-backed queue like Celery/RQ, since there's no Redis/broker instance to actually run and verify one against in this project's dev sandboxes) rather than blocking the request, so the old 200-item cap is now a 10,000-item sanity ceiling instead of a "must fit in one HTTP round trip" limit. If the worker process running a job's thread dies mid-run, that job is stuck at "running" with no supervisor to requeue it — `flask reap-stale-generation-jobs` (see `.env.example`'s `NFT_GENERATION_JOB_STALE_SECONDS`) closes that specific gap by failing a job with no progress in 10 minutes, freeing the collection to be regenerated; it needs to actually run on a schedule to do that, same as `prune-nonces` — as of 2026-09-24 docker-compose.yml's `scheduler` service runs both (`backend/scripts/maintenance-loop.sh`; not yet run under Docker itself, see `docs/OPERATIONS.md`). A real distributed queue still wouldn't need a reaper like this at all — that's the trade-off being managed, not eliminated.
- ~~`backend/Dockerfile`/`frontend/Dockerfile` not build-verified~~ — build- and run-verified for real 2026-09-15 (see "Running the whole stack in Docker instead" above). No longer a gap. Rate limiting itself still defaults to in-memory storage outside Docker (fine for a single dev process) — set `RATE_LIMIT_STORAGE_URI` to a `redis://` URL before running more than one backend worker; `docker compose up -d redis` starts one, and both the wiring and the cross-process sharing it exists for are covered by a real Redis instance in CI (`backend/tests/test_ratelimit_storage.py`) and the 2026-09-15 Docker verification, not mocked.
...
- See each package's `.env.example` for required/optional environment variables, and `docs/OPERATIONS.md` for the production runbook (which variables are actually required per service, migrations, the `/api/health/ready` readiness check, scheduled jobs, RPC providers, what to monitor).
...
**Build-verified for real, 2026-09-15** (see `docs/REBUILD_PROGRESS.md`'s dated entry): all four services build and run together, `flask db upgrade` applies cleanly against a real Postgres container (the first time this project's migrations were ever checked against real Postgres, not just SQLite), nginx's client-side-routing fallback correctly serves the app shell on a hard refresh of a deep route, and a real end-to-end smoke check (real signature → real JWT → a real `Project` row round-tripped through the dockerized Postgres, plus the per-wallet rate limit genuinely enforced through the dockerized Redis) passed. One real bug was found and fixed along the way: `frontend/Dockerfile`'s `npm ci` failed outright, because `@solana/wallet-adapter-wallets` pulls in a Trezor package that depends on `usb`, a native module with no prebuilt binary and no Python/build tools in the Alpine build stage to compile it from source — fixed with `npm ci --ignore-scripts`, since that native binary is for Node-runtime USB access (Electron, etc), not the WebUSB API this browser bundle actually uses.
