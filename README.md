...
- **Automated tests & CI** — pytest (backend, 240+ tests, including a real Redis service container backing the rate-limit storage tests) and Vitest (frontend, 140+ tests), plus real Playwright end-to-end tests for the EVM deploy flow (against a local `anvil` chain), the Solana Candy Machine (including allowlist phases) and SPL token launch flows (against a local `solana-test-validator`, real Metaplex programs cloned onto it), the NFT Generator's own multi-step upload UI and its ERC-721 deploy (read back on-chain, down to each token's `tokenURI` resolving to the right metadata) including real edit/delete/reorder round trips, and a real `@axe-core/playwright` WCAG 2 A/AA scan of every page — see `frontend/e2e/`. Four GitHub Actions jobs (backend, frontend, e2e, Candy Machine sidecar) on every push/PR to `main`.
