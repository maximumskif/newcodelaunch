...
- **Solana liquidity (Raydium)** — "Liquidity" on a launched SPL token: create its Raydium CPMM pool against SOL (0.25% fee tier; Raydium's 0.15 SOL pool-creation fee shown up front), add to it at the pool's price, or withdraw a share of your position — each signed by your own wallet, and recorded only after the backend reads what the pool's vaults did in that transaction.
